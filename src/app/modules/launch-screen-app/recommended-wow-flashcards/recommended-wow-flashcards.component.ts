...
@Component({
  selector: 'app-recommended-wow-flashcards',
  templateUrl: './recommended-wow-flashcards.component.html',
  styleUrls: ['./recommended-wow-flashcards.component.scss']
})
export class RecommendedWowFlashcardsComponent implements OnInit {
  ...
  loadData() {
    this._apiService.GetAllRecommendedWowResourcesListBasedOnLoginId().subscribe({
      next:(res)=>{
        // console.log(res,'kjdskj');
        if(res.data.length>0){
          const flashcardsData = res.data
          flashcardsData.sort((a:any,b:any)=>{
            const dateA:any = new Date(a.shared_datetime);
            const dateB:any = new Date(b.shared_datetime);
  ...
  select_rad_btn(_data:any){
    console.log(_data,'select_rad_btn')
      ...
      `</head>
        ...
        .mat-column-select{display:none}
          ...
          <div style=" display: flex;flex-direction: column; width:100%">
            ...
            <span style="text-align: center;font-size:16px;color:black;font-weight:600;text-transform: uppercase">List of Recommended WOW FlashCards</span>
